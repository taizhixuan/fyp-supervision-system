# Sequence Diagrams with Implementation Analysis

## 4.2.1 UC1 Register and Log In (All Roles)

### Implementation Analysis

**Check current implementation of Register and Log In pages whether matching with the flow stated below:**

**Current Implementation:**
- **RegisterPage.tsx** (`frontend/src/pages/auth/RegisterPage.tsx`): Implements registration form with role selection (Student/Supervisor only), full name, MMU ID (10-digit validation), email, phone (optional), password with strength requirements, confirm password, and terms acceptance.
- **LoginPage.tsx** (`frontend/src/pages/auth/LoginPage.tsx`): Implements login form with MMU ID or email identifier, password, and "Remember me" option.
- **AuthContext.tsx** (`frontend/src/lib/auth/AuthContext.tsx`): Manages authentication state, provides `login()` and `register()` functions, stores JWT token in localStorage.
- **auth.ts API** (`frontend/src/lib/api/auth.ts`): Defines `POST /auth/register` and `POST /auth/login` endpoints.
- **Validators** (`frontend/src/lib/validators/auth.ts`): Zod schemas for form validation including MMU ID format (10 digits), password requirements (min 8 chars, uppercase, number).

**What Matches the Sequence Diagram Flow:**
1. User opens register/login page -> FE shows form (Steps 1-2)
2. Fill registration form -> POST /auth/register (Steps 3-4)
3. Role restriction: Only Student/Supervisor can self-register; FYP Committee/System Admin roles are blocked in UI (Steps 27-37)
4. Login with MMU ID and password -> POST /auth/login (Steps 41-42)
5. Success returns token and role -> redirect to role dashboard (Steps 51-55)
6. Invalid credentials show error message (Steps 46-49)

**What is Missing/Needs Implementation:**
1. **Backend Integration**: Currently using mock auth in development mode - needs real backend API
2. **Duplicate Check**: Backend should check MMU ID and email uniqueness (Steps 21-25) - frontend only shows generic error
3. **Audit Logging**: Backend needs to log registration/login success events (Steps 30-31, 52-53)
4. **Specific Error Messages**: Show "duplicate MMU ID" or "duplicate email" errors instead of generic message
5. **Account Pending Flow**: After registration, user redirects to `/account-pending` - backend approval workflow needed

---

### Mermaid Sequence Diagram (UC1)
...
**Check current implementation of Student Profile page whether matching with the flow stated below:**

**Current Implementation:**
- **StudentProfile.tsx** (`frontend/src/pages/student/StudentProfile.tsx`): Comprehensive profile page with view/edit modes for personal info, academic details, bio, skills, and research interests.
- Uses `useStudentProfile()` hook to fetch profile data
- Uses `useUpdateStudentProfile()` mutation to save changes
- Uses `useUploadProfileImage()` mutation to upload profile picture
- Features: Avatar with upload functionality (camera icon in edit mode), social links (LinkedIn, GitHub, Portfolio), skills management with add/remove, research interests management
- Profile picture upload includes: file type validation (JPEG, PNG, GIF, WebP), size limit (5MB), preview modal before upload
- Skills and research interests flagged as "Used for AI matching"

**What Matches the Sequence Diagram Flow:**
1. Student opens Profile page -> GET /student/profile (Steps 1-2)
2. Backend validates token and role (Steps 3-4)
3. Profile data displayed in form (Steps 5-10)
4. Cancel discards edits (Steps 12-14 - A1 flow)
5. Save changes -> PUT /student/profile (Steps 15-17)
6. Validation errors shown to user (Steps 21-24 - E1 flow)
7. Success message displayed (Steps 25-32)
8. Profile picture upload -> POST /student/profile/image (multipart/form-data)

**What is Missing/Needs Implementation:**
1. **Backend API**: Real API endpoints `GET /student/profile`, `PUT /student/profile`, and `POST /student/profile/image` need to be implemented
2. **Audit Logging**: Profile update events should be logged (Steps 27-28)
3. **Server-side Validation**: Backend validation rules need to match frontend Zod schemas
4. **File Storage**: Backend needs file storage service for profile images (S3, local storage, etc.)
...
**Check current implementation of FYP Dashboard page whether matching with the flow stated below:**

**Current Implementation:**
- **StudentDashboard.tsx** (`frontend/src/pages/student/StudentDashboard.tsx`): Comprehensive dashboard with welcome header, quick stats, registration progress tracker, proposal status, upcoming deadlines, meetings, documents, and quick actions.
- Uses `useStudentDashboard()` hook to fetch all dashboard data in one call
- Features: Quick stats cards (meetings, logs, documents), pending logs alert, registration progress steps, proposal status with AI analysis link, deadline countdown, upcoming meetings grid, recent documents list

**What Matches the Sequence Diagram Flow:**
1. Student opens Dashboard -> GET /student/dashboard (Steps 1-2)
2. Backend validates session (Steps 3-4)
3. Dashboard loads summary data: proposal status, meetings, logs, documents (Steps 6-12)
4. Load deadline reminders and notifications (Steps 13-14)
5. Display dashboard cards and alerts (Steps 15-17)
6. If no project, display guidance and next steps (Steps 19-24 - A1 flow)

**What is Missing/Needs Implementation:**
1. **Backend API**: Real `GET /student/dashboard` endpoint needs to aggregate data from multiple services
2. **Notification Service Integration**: Dashboard should show real-time notifications (Steps 13-14)
3. **Error Handling**: Data retrieval error logging and display (Steps 27-32 - E1 flow)
4. **Empty State**: Currently shows sample data; needs proper "no project" guidance state
...
**Check current implementation of Browse and Search Supervisors page whether matching with the flow stated below:**

**Current Implementation:**
- **SupervisorDirectory.tsx** (`frontend/src/pages/student/SupervisorDirectory.tsx`): Full-featured supervisor directory with search, filters, and compare functionality.
- Uses `useSupervisorList()` hook with filter parameters (search, faculty, researchArea, availableOnly)
- Features: Search bar, faculty dropdown, research areas multi-select, availability filter, supervisor cards with details, compare up to 3 supervisors, View Profile and Request buttons

**What Matches the Sequence Diagram Flow:**
1. Student opens Supervisor Directory -> GET /supervisors with filters (Steps 1-2)
2. Backend validates token and role (Steps 3-4)
3. Search supervisors with filters from database (Steps 5-8)
4. Display list and filter controls (Steps 9-10)
5. Refine search with updated filters (Steps 12-22)
6. Refresh results on filter change (Steps 23)

**What is Missing/Needs Implementation:**
1. **Backend API**: Real `GET /supervisors` endpoint with query parameters for filters
2. **Error Handling**: Data retrieval error logging (Steps 25-31 - error flow)
3. **Pagination**: Large result sets need pagination support
4. **Real-time Availability**: Supervisor availability should reflect current slot status

---

### Mermaid Sequence Diagram (UC4)
...
**Check current implementation of AI Supervisor Recommendations page whether matching with the flow stated below:**

**Current Implementation:**
- **AIRecommendations.tsx** (`frontend/src/pages/student/AIRecommendations.tsx`): AI-powered supervisor recommendations based on student profile and proposal topic.
- Links from Dashboard and Supervisor Directory pages
- Uses student skills and research interests (marked as "Used for AI matching" in profile)

**What Matches the Sequence Diagram Flow:**
1. Student opens AI Recommendations page -> GET /student/recommendations (Steps 1-2)
2. Backend validates token and role (Steps 3-4)
3. Load student profile and proposal topic (Steps 6-7)
4. If proposal topic exists, call AI service for recommendations (Steps 9-14)
5. Display ranked supervisor recommendations (Steps 15-16)
6. If no proposal topic, show guidance message (Steps 17-19)

**What is Missing/Needs Implementation:**
1. **AI Service Integration**: Real AI recommendation service endpoint `/recommendSupervisor`
2. **Backend API**: `GET /student/recommendations` endpoint that aggregates student data and calls AI
3. **Scoring Display**: Show match scores/percentages from AI analysis
4. **Proposal Topic Validation**: Check if student has submitted proposal topic before showing recommendations

---

### Mermaid Sequence Diagram (UC5)
...
**Check current implementation of Send Supervisor Request page whether matching with the flow stated below:**

**Current Implementation:**
- **CreateSupervisionRequest.tsx** (`frontend/src/pages/student/CreateSupervisionRequest.tsx`): Form to send supervision request to a selected supervisor.
- **MyRequests.tsx** (`frontend/src/pages/student/MyRequests.tsx`): Track status of submitted requests.
- Request button available on SupervisorDirectory and SupervisorDetail pages
- Uses `useCreateSupervisionRequest()` mutation

**What Matches the Sequence Diagram Flow:**
1. Student selects supervisor and clicks Request (Steps 1-2)
2. POST /supervisor-requests sent (Steps 3)
3. Backend validates token and role (Steps 4-5)
4. Check existing request and eligibility (Steps 7-8)
5. If not allowed, show rejection reason (Steps 10-13)
6. If allowed, create request and notify supervisor (Steps 15-21)
7. Show request submitted status (Steps 26)

**What is Missing/Needs Implementation:**
1. **Backend API**: `POST /supervisor-requests` endpoint with eligibility checks
2. **Eligibility Rules**: Backend logic for checking existing requests, supervisor capacity, student status
3. **Notification Service**: Real-time notification to supervisor about new request
4. **Audit Logging**: Log request creation events

---

### Mermaid Sequence Diagram (UC6)
...
**Check current implementation of Manage Proposal page whether matching with the flow stated below:**

**Current Implementation:**
- **ProposalWorkspace.tsx** (`frontend/src/pages/student/ProposalWorkspace.tsx`): Full proposal editing workspace with form fields for title, problem statement, objectives, scope, methodology, expected outcomes, timeline, and references.
- Uses `useCurrentProposal()`, `useCreateProposal()`, `useUpdateProposal()`, `useSubmitProposal()`, `useUploadProposalFile()` hooks
- Features: Status badge with version, action bar (History, AI Analysis, Status), Save Draft button, Submit button, file upload (PDF/DOC/DOCX max 10MB), submit confirmation modal

**What Matches the Sequence Diagram Flow:**
1. Student opens Proposal page -> GET /student/proposal (Steps 1-2)
2. Display proposal form and status (Steps 9-11)
3. Save draft -> PUT /student/proposal (Steps 13-26)
4. Upload proposal file -> POST /student/proposal/file (Steps 28-41)
5. Submit proposal -> POST /student/proposal/submit (Steps 43-67)
6. AI analysis triggered on submit (Steps 52-59)
7. Notify supervisor about submission (Steps 61-62)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: All proposal endpoints need implementation
2. **Version Control**: Backend should create proposal versions on each save/submit
3. **AI Proposal Analyzer**: Integration with AI service for proposal analysis
4. **File Storage**: Backend file storage service for proposal documents
5. **Notification Service**: Notify supervisor on submission

---

### Mermaid Sequence Diagram (UC7)
...
**Check current implementation of View Proposal Status page whether matching with the flow stated below:**

**Current Implementation:**
- **ProposalStatus.tsx** (`frontend/src/pages/student/ProposalStatus.tsx`): Status timeline view showing proposal review progress and feedback.
- Accessible from ProposalWorkspace action bar
- Shows status history, supervisor feedback, and revision requirements

**What Matches the Sequence Diagram Flow:**
1. Student opens Proposal Status page -> GET /student/proposal/status (Steps 1-2)
2. Load proposal status timeline and feedback (Steps 6-9)
3. Display status timeline and feedback (Steps 10-11)
4. If revision requested, display required changes (Steps 13-14 - A1)

**What is Missing/Needs Implementation:**
1. **Backend API**: `GET /student/proposal/status` endpoint
2. **Timeline Data**: Backend should return status history with timestamps
3. **Feedback Display**: Show supervisor comments and required changes
4. **Edit Link**: Provide link back to proposal editor when revision needed

---

### Mermaid Sequence Diagram (UC8)
...
**Check current implementation of View Project Registration Status page whether matching with the flow stated below:**

**Current Implementation:**
- **RegistrationStatus.tsx** (`frontend/src/pages/student/RegistrationStatus.tsx`): FYP registration phase tracking page.
- Shows current FYP stage and supervisor pairing status
- Displays next steps and guidance based on current status

**What Matches the Sequence Diagram Flow:**
1. Student opens Registration Status page -> GET /student/registration/status (Steps 1-2)
2. Load FYP stage and pairing status (Steps 6-8)
3. Display FYP stage and pairing info (Steps 10-11)
4. If not registered, display registration guidance (Steps 13-14 - A1)

**What is Missing/Needs Implementation:**
1. **Backend API**: `GET /student/registration/status` endpoint
2. **Stage Tracking**: Backend logic for FYP phases (Find Supervisor, Submit Proposal, Committee Review, etc.)
3. **Status Consistency Check**: Validate data consistency and show warnings if needed (E1 flow)

---

### Mermaid Sequence Diagram (UC9)
...
**Check current implementation of Manage Meeting Schedule page whether matching with the flow stated below:**

**Current Implementation:**
- **MeetingList.tsx** (`frontend/src/pages/student/MeetingList.tsx`): Main meetings page with list/grid view, status filters, and meeting cards.
- **MeetingRequest.tsx** (`frontend/src/pages/student/MeetingRequest.tsx`): Form to request new meeting.
- **MeetingDetail.tsx** (`frontend/src/pages/student/MeetingDetail.tsx`): Detailed view of a meeting.
- **MeetingExport.tsx** (`frontend/src/pages/student/MeetingExport.tsx`): Export meetings to calendar/PDF.
- Uses `useMeetingList()` hook, features: quick stats cards, status filters, meeting cards with date/time/platform/agenda

**What Matches the Sequence Diagram Flow:**
1. Open Meeting Schedule module -> GET /student/meetings (Steps 1-2)
2. Display calendar or list view (Steps 10)
3. Request meeting with time, platform, agenda (Steps 14-17)
4. Validate policy rules (Steps 20-28)
5. Create pending request and notify supervisor (Steps 29-38)
6. Edit pending request (Steps 42-53 - A1)
7. Cancel pending request (Steps 55-66 - A2)
8. View meeting details (Steps 68-77 - A3)
9. Export schedule (Steps 79-85 - A4)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: All meeting-related endpoints
2. **Policy Validator**: Meeting time policy validation rules
3. **Notification Service**: Notify supervisor on meeting requests/changes
4. **Export Service**: Generate calendar/PDF exports
5. **Pairing Check**: Disable request action if student not paired with supervisor (E2)

---

### Mermaid Sequence Diagram (UC10)
...
**Check current implementation of Manage Supervision Log page whether matching with the flow stated below:**

**Current Implementation:**
- **LogList.tsx** (`frontend/src/pages/student/LogList.tsx`): Main logs page with search, status filters, and log cards grouped chronologically.
- **LogCreate.tsx** (`frontend/src/pages/student/LogCreate.tsx`): Form to create new supervision log.
- **LogDetail.tsx** (`frontend/src/pages/student/LogDetail.tsx`): Detailed view of a log with feedback.
- **LogEdit.tsx** (`frontend/src/pages/student/LogEdit.tsx`): Edit existing log.
- Uses `useLogList()` hook, features: quick stats (total, approved, pending, drafts), status filters, log cards with progress bar, meeting link badge, supervisor feedback

**What Matches the Sequence Diagram Flow:**
1. Open Supervision Log module -> GET /student/logs (Steps 1-2)
2. Display log history (Steps 10)
3. Create or upload log (Steps 12-31)
4. If submitted, notify supervisor (Steps 33-35)
5. Edit and resubmit for correction (Steps 38-50 - A3)
6. Sign log (Steps 52-75)
7. Lock log when both signatures exist (Steps 68-71)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: All log-related endpoints
2. **File Storage**: Log file upload functionality
3. **Signature System**: Digital signature capture and storage
4. **Log Locking**: Backend logic to lock logs when both parties sign
5. **Notification Service**: Notify supervisor on log submission/resubmission

---

### Mermaid Sequence Diagram (UC11)
...
**Check current implementation of Upload and Manage FYP Documents page whether matching with the flow stated below:**

**Current Implementation:**
- **DocumentList.tsx** (`frontend/src/pages/student/DocumentList.tsx`): Main documents page with category stats, search, list/grid view toggle.
- **DocumentUpload.tsx** (`frontend/src/pages/student/DocumentUpload.tsx`): Form to upload new documents.
- **DocumentDetail.tsx** (`frontend/src/pages/student/DocumentDetail.tsx`): Detailed view of a document.
- **DocumentHistory.tsx** (`frontend/src/pages/student/DocumentHistory.tsx`): Version history of documents.
- Uses `useDocumentList()` hook, features: category filter (Proposal, Report, Diagram, Code, Presentation, Other), file details, version badges

**What Matches the Sequence Diagram Flow:**
1. Open Document module -> GET /student/documents (Steps 1-2)
2. Display documents list (Steps 10)
3. Select document type and phase, upload file (Steps 12-15)
4. Validate file rules and scan (Steps 17-18)
5. If rejected, show reason (Steps 20-23)
6. If accepted, store file and save record (Steps 25-33)
7. Replace document with version history (Steps 36-47 - A1)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: All document-related endpoints
2. **File Storage**: Backend file storage service
3. **File Scanning**: Virus/malware scan before accepting files
4. **Version Control**: Backend version history management
5. **Audit Logging**: Log document upload events

---

### Mermaid Sequence Diagram (UC12)
...
**Check current implementation of View FYP Guidelines, Rubrics and Deadlines page whether matching with the flow stated below:**

**Current Implementation:**
- **ResourcesHub.tsx** (`frontend/src/pages/student/ResourcesHub.tsx`): Repository of FYP resources and templates.
- **ResourceDetail.tsx** (`frontend/src/pages/student/ResourceDetail.tsx`): Detailed view of a resource.
- **DeadlineCalendar.tsx** (`frontend/src/pages/student/DeadlineCalendar.tsx`): Visual calendar of all deadlines.
- Resources accessible via Dashboard and navigation

**What Matches the Sequence Diagram Flow:**
1. Open Guidelines/Rubrics/Deadlines page -> GET /resources (Steps 1-2)
2. Display resources list (Steps 10)
3. Keyword search (Steps 12-21 - A1)
4. Open or download resource (Steps 23-24)
5. Fetch file from storage (Steps 29-33)
6. Handle missing file error (Steps 26-30 - E1)

**What is Missing/Needs Implementation:**
1. **Backend API**: `GET /resources` with category filtering and search
2. **File Storage**: Backend file retrieval service
3. **Audit Logging**: Log missing file incidents
4. **Category Organization**: Guidelines, Rubrics, Deadlines categories

---

### Mermaid Sequence Diagram (UC13)
...
**Check current implementation of View Reminders and Notifications page whether matching with the flow stated below:**

**Current Implementation:**
- **NotificationCenter.tsx** (`frontend/src/pages/student/NotificationCenter.tsx`): Centralized notification hub with filtering and read status management.
- **NotificationSettings.tsx** (`frontend/src/pages/student/NotificationSettings.tsx`): Notification preferences configuration.
- Uses `useNotifications()`, `useMarkNotificationRead()`, `useMarkAllRead()` hooks
- Features: Unread count, Mark All Read, type filters (Info, Success, Warning, Error, Meeting, Deadline, Feedback, System), notification cards with time ago

**What Matches the Sequence Diagram Flow:**
1. Trigger events create notifications (Steps 1-4)
2. Delivery failure handling with retry (Steps 6-9)
3. Open Notifications page -> GET /student/notifications (Steps 11-13)
4. Display reminders and notifications (Steps 17-18)
5. Update notification preferences (Steps 20-27 - A1)

**What is Missing/Needs Implementation:**
1. **Notification Service**: Backend service to create/manage notifications
2. **Trigger Events**: System events that generate notifications (deadlines, meetings, announcements)
3. **Delivery Mechanism**: Email/push notification delivery
4. **Retry Logic**: Handle delivery failures with scheduled retry

---

### Mermaid Sequence Diagram (UC14)
...
**Check current implementation of FYP Chatbot page whether matching with the flow stated below:**

**Current Implementation:**
- **Chatbot.tsx** (`frontend/src/pages/student/Chatbot.tsx`): AI chatbot interface for FYP assistance.
- Chat-style interface for asking questions about FYP process, guidelines, deadlines

**What Matches the Sequence Diagram Flow:**
1. Open Chatbot -> GET /chat/session (Steps 1-2)
2. Create or load chat session (Steps 6-11)
3. Display chat interface (Steps 13)
4. Ask question -> POST /chat/message (Steps 15-16)
5. Save user message (Steps 19-21)
6. Send to AI chatbot and get response (Steps 24-30)
7. Display answer and reference (Steps 32)
8. Handle chatbot unavailable (Steps 34-38 - E1)
9. Low confidence answer suggestion (Steps 40-42 - A1)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Chat session and message endpoints
2. **AI Chatbot Service**: Integration with AI service for answering FYP questions
3. **Knowledge Base**: FYP guidelines, rubrics, and FAQs for chatbot context
4. **Confidence Scoring**: Detect low-confidence answers and suggest human contact
5. **Session Persistence**: Store chat history for continuity

---

### Mermaid Sequence Diagram (UC15)
...
**Check current implementation of Supervisor Profile page whether matching with the flow stated below:**

**Current Implementation:**
- **SupervisorProfile.tsx** (`frontend/src/pages/supervisor/`): Profile management page for supervisors.
- Expected to include: Research areas, supervision quota, availability settings, bio, contact info
- Uses similar pattern to StudentProfile with view/edit modes

**What Matches the Sequence Diagram Flow:**
1. Supervisor opens Profile page -> GET /supervisor/profile (Steps 1-2)
2. Display profile form (Steps 10)
3. Cancel discards edits (Steps 12-14)
4. Save changes -> PUT /supervisor/profile (Steps 16-18)
5. Validation and update (Steps 19-30)

**What is Missing/Needs Implementation:**
1. **Backend API**: `GET /supervisor/profile` and `PUT /supervisor/profile` endpoints
2. **Research Areas Management**: Add/remove research areas
3. **Quota Settings**: Set maximum supervisee count
4. **Availability Toggle**: Set availability for new students
5. **Audit Logging**: Log profile update events

---

### Mermaid Sequence Diagram (UC16)
...
**Check current implementation of Supervisor Requests page whether matching with the flow stated below:**

**Current Implementation:**
- **RequestList.tsx** (`frontend/src/pages/supervisor/`): View incoming supervision requests from students.
- **RequestDetail.tsx** (`frontend/src/pages/supervisor/`): Detailed view with approve/reject actions.
- Features: Request cards with student info, proposal summary, approve/reject buttons

**What Matches the Sequence Diagram Flow:**
1. Open Supervisor Requests page -> GET /supervisor/requests (Steps 1-2)
2. Display requests list (Steps 10)
3. Open request details (Steps 12-20)
4. Approve request (Steps 22-37)
5. Reject request with reason (Steps 38-51)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Request management endpoints
2. **Pairing Logic**: Create student-supervisor pairing on approval
3. **Notification Service**: Notify student of approval/rejection
4. **Quota Enforcement**: Check supervisor capacity before approval
5. **Audit Logging**: Log request decisions

---

### Mermaid Sequence Diagram (UC17)
...
**Check current implementation of Supervisee List page whether matching with the flow stated below:**

**Current Implementation:**
- **SuperviseeList.tsx** (`frontend/src/pages/supervisor/`): List of assigned supervisees with project summaries.
- **SuperviseeDetail.tsx** (`frontend/src/pages/supervisor/`): Detailed view of a supervisee's project.
- Features: Supervisee cards with progress indicators, proposal status, meeting count, log status

**What Matches the Sequence Diagram Flow:**
1. Open Supervisee List page -> GET /supervisor/supervisees (Steps 1-2)
2. Display supervisee list (Steps 10)
3. Select supervisee project (Steps 12-13)
4. Load and display project details (Steps 14-21)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Supervisee list and detail endpoints
2. **Project Summary**: Aggregated data for proposal, meetings, logs, documents
3. **Progress Tracking**: Visual progress indicators
4. **Audit Logging**: Log error events

---

### Mermaid Sequence Diagram (UC18)
...
**Check current implementation of Review Student Proposal page whether matching with the flow stated below:**

**Current Implementation:**
- **ProposalList.tsx** (`frontend/src/pages/supervisor/`): List of submitted proposals from supervisees.
- **ProposalReview.tsx** (`frontend/src/pages/supervisor/`): Detailed review page with feedback form.
- Features: Proposal details, AI analysis results, comment input, approve/reject/revision buttons

**What Matches the Sequence Diagram Flow:**
1. Open Proposal Review page -> GET /supervisor/proposals (Steps 1-2)
2. Display proposal list (Steps 10)
3. Open proposal details with versions and AI results (Steps 12-23)
4. Submit feedback and decision (Steps 25-39)
5. Refresh AI analysis (Steps 41-53)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Proposal review endpoints
2. **AI Integration**: Refresh AI analysis on demand
3. **Decision Workflow**: Approve/reject/revision status updates
4. **Notification Service**: Notify student of decision
5. **File Storage**: Access proposal files

---

### Mermaid Sequence Diagram (UC19)
...
**Check current implementation of Manage Supervision Meetings page whether matching with the flow stated below:**

**Current Implementation:**
- **MeetingList.tsx** (`frontend/src/pages/supervisor/`): View meeting requests and scheduled meetings.
- **MeetingDetail.tsx** (`frontend/src/pages/supervisor/`): Detailed meeting view with confirm/reschedule actions.
- Features: Meeting cards, pending requests, confirmed meetings, reschedule form

**What Matches the Sequence Diagram Flow:**
1. Open Meetings module -> GET /supervisor/meetings (Steps 1-2)
2. Display meeting list (Steps 10)
3. Open meeting request details (Steps 12-20)
4. Confirm meeting (Steps 22-35)
5. Reschedule meeting (Steps 36-50)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Meeting management endpoints
2. **Notification Service**: Notify student on confirm/reschedule
3. **Calendar Integration**: Sync with external calendars
4. **Audit Logging**: Log meeting decisions

---

### Mermaid Sequence Diagram (UC20)
...
**Check current implementation of Supervision Log Review page whether matching with the flow stated below:**

**Current Implementation:**
- **LogList.tsx** (`frontend/src/pages/supervisor/`): View submitted logs from supervisees.
- **LogReview.tsx** (`frontend/src/pages/supervisor/`): Detailed log review with comment and sign actions.
- Features: Log content view, file download, comment input, request correction, sign button

**What Matches the Sequence Diagram Flow:**
1. Open Supervision Logs module -> GET /supervisor/logs (Steps 1-2)
2. Display logs list (Steps 10)
3. Open log details with file link (Steps 12-22)
4. Request correction with comments (Steps 24-37)
5. Sign log with lock logic (Steps 38-59)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Log review endpoints
2. **Signature System**: Digital signature capture for supervisor
3. **Log Locking**: Lock log when both signatures exist
4. **Notification Service**: Notify student on correction request
5. **File Storage**: Access log files

---

### Mermaid Sequence Diagram (UC21)
...
**Check current implementation of Supervisee Progress Dashboard page whether matching with the flow stated below:**

**Current Implementation:**
- **SupervisorDashboard.tsx** (`frontend/src/pages/supervisor/`): Main dashboard with overview of all supervisees' progress.
- Features: Quick stats, supervisee progress cards, alerts for pending actions

**What Matches the Sequence Diagram Flow:**
1. Open Supervisee Progress Dashboard -> GET /supervisor/progress (Steps 1-2)
2. Load progress summary for supervisees (Steps 6-10)
3. Display progress cards and alerts (Steps 12)

**What is Missing/Needs Implementation:**
1. **Backend API**: `GET /supervisor/progress` aggregating supervisee data
2. **Progress Metrics**: Proposal status, meeting count, log status, document count
3. **Alerts**: Identify students needing attention
4. **Audit Logging**: Log error events

---

### Mermaid Sequence Diagram (UC22)
...
**Check current implementation of FYP Documents page whether matching with the flow stated below:**

**Current Implementation:**
- **DocumentList.tsx** (`frontend/src/pages/supervisor/`): View supervisee documents.
- **DocumentDetail.tsx** (`frontend/src/pages/supervisor/`): Download and review documents with feedback upload.
- Features: Document list, download button, feedback upload form

**What Matches the Sequence Diagram Flow:**
1. Open Documents module -> GET /supervisor/documents (Steps 1-2)
2. Display documents list (Steps 10)
3. Download document (Steps 12-19)
4. Upload feedback file (Steps 21-35)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Document management endpoints
2. **File Storage**: Download and upload file services
3. **Notification Service**: Notify student of feedback upload
4. **Audit Logging**: Log feedback upload events

---

### Mermaid Sequence Diagram (UC23)
...
**Check current implementation of Publish FYP Announcements page whether matching with the flow stated below:**

**Current Implementation:**
- **AnnouncementList.tsx** (`frontend/src/pages/supervisor/` and `frontend/src/pages/committee/`): View and manage announcements.
- **AnnouncementCreate.tsx**: Form to create new announcements.
- **AnnouncementEdit.tsx**: Edit existing announcements.
- Features: Announcement cards, create form with title/content/scope/audience, schedule option

**What Matches the Sequence Diagram Flow:**
1. Open Announcements module -> GET /announcements/manage (Steps 1-2)
2. Display announcements list (Steps 10)
3. Create announcement with title, content, scope, audience (Steps 12-14)
4. Validate and publish (Steps 15-29)
5. Send notifications (Steps 21-22)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Announcement management endpoints
2. **Scope/Audience Logic**: Target specific groups (students, supervisors, all)
3. **Scheduling**: Delayed publication based on schedule
4. **Notification Service**: Push announcements to targeted users
5. **Audit Logging**: Log announcement publication

---

### Mermaid Sequence Diagram (UC24)
...
**Check current implementation of Proposal Review Queue page whether matching with the flow stated below:**

**Current Implementation:**
- **ProposalQueue.tsx** (`frontend/src/pages/committee/`): Queue of proposals pending committee review.
- Features: Proposal list with filters (status, faculty, date), search functionality

**What Matches the Sequence Diagram Flow:**
1. Open Proposal Review Queue -> GET /committee/proposals/queue (Steps 1-2)
2. Display queue list with filters (Steps 10)
3. Apply filter or search (Steps 12-22)

**What is Missing/Needs Implementation:**
1. **Backend API**: `GET /committee/proposals/queue` with filter parameters
2. **Queue Management**: Priority ordering, assignment tracking
3. **Filter Logic**: By status, faculty, submission date
4. **Audit Logging**: Log error events

---

### Mermaid Sequence Diagram (UC25)
...
**Check current implementation of Review Proposal page whether matching with the flow stated below:**

**Current Implementation:**
- **ProposalReview.tsx** (`frontend/src/pages/committee/`): Detailed proposal review with decision form.
- Features: Proposal details, supervisor review history, committee decision (approve/reject/revision), comments

**What Matches the Sequence Diagram Flow:**
1. Open proposal from queue -> GET /committee/proposals/{proposalId} (Steps 1-2)
2. Display proposal details and status (Steps 10-14)
3. Enter comments and decision (Steps 16-17)
4. Validate and save review (Steps 18-33)
5. Notify student and supervisor (Steps 26-27)

**What is Missing/Needs Implementation:**
1. **Backend API**: Committee proposal review endpoint
2. **Decision Workflow**: Final approval/rejection authority
3. **Notification Service**: Notify all stakeholders
4. **File Storage**: Access proposal files
5. **Audit Logging**: Log committee decisions

---

### Mermaid Sequence Diagram (UC26)
...
**Check current implementation of Manage General FYP Documents page whether matching with the flow stated below:**

**Current Implementation:**
- **DocumentList.tsx** (`frontend/src/pages/committee/`): Manage FYP resources and templates.
- **DocumentUpload.tsx** (`frontend/src/pages/committee/`): Upload new resources.
- Features: Category organization (Guidelines, Rubrics, Templates), upload form, metadata editing

**What Matches the Sequence Diagram Flow:**
1. Open General FYP Documents module -> GET /committee/resources (Steps 1-2)
2. Display resources (Steps 10)
3. Upload new resource (Steps 12-30)
4. Update resource metadata (Steps 32-41)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Resource management endpoints
2. **File Storage**: Store and retrieve resource files
3. **Category Management**: Organize by type (Guidelines, Rubrics, Templates, Deadlines)
4. **Audit Logging**: Log upload and update events

---

### Mermaid Sequence Diagram (UC27)
...
**Check current implementation of FYP Project and Pairing Overview page whether matching with the flow stated below:**

**Current Implementation:**
- **ProjectOverview.tsx** (`frontend/src/pages/committee/`): Dashboard showing project and pairing statistics.
- Features: Total projects, paired/unpaired counts, supervisor load distribution, filters by cycle/status

**What Matches the Sequence Diagram Flow:**
1. Open Project and Pairing Overview -> GET /committee/overview (Steps 1-2)
2. Display overview dashboard (Steps 10)
3. Apply filters (Steps 12-22)

**What is Missing/Needs Implementation:**
1. **Backend API**: `GET /committee/overview` with aggregated metrics
2. **Metrics Calculation**: Project counts, pairing status, supervisor load
3. **Filter Support**: By FYP cycle, status, unpaired students
4. **Audit Logging**: Log error events

---

### Mermaid Sequence Diagram (UC28)
...
**Check current implementation of Generate and Export FYP Reports page whether matching with the flow stated below:**

**Current Implementation:**
- **Reports.tsx** (`frontend/src/pages/committee/`): Report generation interface.
- **ReportHistory.tsx** (`frontend/src/pages/committee/`): Previously generated reports.
- Features: Report type selection, filter parameters, generate button, download link

**What Matches the Sequence Diagram Flow:**
1. Open Reports module -> GET /committee/reports (Steps 1-2)
2. Display report filters and types (Steps 5-6)
3. Select report type and filters, click Generate (Steps 8-9)
4. Build dataset and generate report file (Steps 11-17)
5. Download report file (Steps 21)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Report generation endpoints
2. **Report Types**: Progress reports, pairing reports, completion reports
3. **Export Formats**: PDF, Excel, CSV
4. **File Storage**: Store generated reports
5. **Audit Logging**: Log report generation events

---

### Mermaid Sequence Diagram (UC29)
...
**Check current implementation of Manage User Accounts page whether matching with the flow stated below:**

**Current Implementation:**
- **UserList.tsx** (`frontend/src/pages/admin/`): List of all user accounts.
- **UserCreate.tsx** (`frontend/src/pages/admin/`): Create new user account.
- **UserDetail.tsx** (`frontend/src/pages/admin/`): View user details.
- **UserEdit.tsx** (`frontend/src/pages/admin/`): Edit user role and status.
- Features: User search/filter, create form, role selection, activate/deactivate toggle

**What Matches the Sequence Diagram Flow:**
1. Open User Management module -> GET /admin/users (Steps 1-2)
2. Display users list and actions (Steps 10)
3. Create user account (Steps 12-33)
4. Update role or status (Steps 35-52)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: User management endpoints
2. **Role Assignment**: Create FYP Committee and System Admin accounts
3. **Notification Service**: Send account credentials/invite
4. **Status Management**: Activate/deactivate accounts
5. **Audit Logging**: Log all user management actions

---

### Mermaid Sequence Diagram (UC30)
...
**Check current implementation of Configure System Parameters page whether matching with the flow stated below:**

**Current Implementation:**
- **Parameters.tsx** (`frontend/src/pages/admin/`): System configuration parameters.
- Features: Parameter list with current values, edit form, save button

**What Matches the Sequence Diagram Flow:**
1. Open System Parameters module -> GET /admin/config/parameters (Steps 1-2)
2. Display parameters (Steps 10)
3. Edit parameter values (Steps 12-13)
4. Validate and save (Steps 14-25)

**What is Missing/Needs Implementation:**
1. **Backend API**: `GET /admin/config/parameters` and `PUT /admin/config/parameters`
2. **Parameter Types**: Define configurable system parameters
3. **Validation Rules**: Parameter-specific validation logic
4. **Audit Logging**: Log configuration changes

---

### Mermaid Sequence Diagram (UC31)
...
**Check current implementation of Configure Integration and Export Settings page whether matching with the flow stated below:**

**Current Implementation:**
- **Integrations.tsx** (`frontend/src/pages/admin/`): External system integration settings.
- **ExportSettings.tsx** (`frontend/src/pages/admin/`): Export configuration.
- Features: API endpoint configuration, export rules, test connection button

**What Matches the Sequence Diagram Flow:**
1. Open Integration and Export Settings -> GET /admin/config/integrations (Steps 1-2)
2. Display settings (Steps 10)
3. Update API keys, endpoints, export rules (Steps 12-13)
4. Optional test connection (Steps 15-18)
5. Validate and save (Steps 20-30)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Integration configuration endpoints
2. **Connection Testing**: Verify external service connectivity
3. **Export Rules**: Define export formats and schedules
4. **Audit Logging**: Log configuration changes

---

### Mermaid Sequence Diagram (UC32)
...
**Check current implementation of Perform System Maintenance page whether matching with the flow stated below:**

**Current Implementation:**
- **Maintenance.tsx** (`frontend/src/pages/admin/`): System maintenance actions.
- **MaintenanceJobs.tsx** (`frontend/src/pages/admin/`): Scheduled maintenance jobs.
- **AuditLogs.tsx** (`frontend/src/pages/admin/`): View audit log history.
- Features: Backup button, health check, audit log viewer

**What Matches the Sequence Diagram Flow:**
1. Open System Maintenance module -> GET /admin/maintenance (Steps 1-2)
2. Display maintenance actions (Steps 5-6)
3. Backup database and files (Steps 8-21)
4. View audit logs (Steps 23-30)
5. System health check (Steps 32-38)

**What is Missing/Needs Implementation:**
1. **Backend APIs**: Maintenance action endpoints
2. **Backup Service**: Database and file backup functionality
3. **Health Check**: System component status monitoring
4. **Audit Log Query**: Search and filter audit logs
5. **Job Scheduling**: Background maintenance jobs

---

### Mermaid Sequence Diagram (UC33)
